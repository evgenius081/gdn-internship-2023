...
export function MinMaxRate(){
    ...
    async function send(e){
        ...
        await fetch(process.env.REACT_APP_ASP_LINK+"/minmax/"+currency.code+"/"+quotationNumber)
            .then(async (response) => {
                if (response.ok){
                    let data = await response.json()
                    setStatus("Min: "+data.item1+" Max: "+data.item2)
                }
    ...
    return (
        <>
            <h1>Min and max currency rates to PLN</h1>
            <form>
                <div className="form-group">
                    <div className="form-group">
                        <label htmlFor="deadline-input">Currency</label>
                        <Dropdown  className="btn-group">
                            <Dropdown.Toggle type="button" className="btn dropdown-toggle" data-toggle="dropdown"
                    ...
                    <div className="form-group">
                        ...
                        <input type="number" className="form-control" value={quotationNumber}
                               onChange={e => setQuotationNumber(e.target.value)}
                               id="quoattion-input" min={1} max={255} required={true}/>
                    </div>
                </div>
